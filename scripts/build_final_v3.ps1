# build_final_v3.ps1
# Enhances matching logic: if distributor/fabricante doesn't yield a provider folder, fall back to matching with the Material/Concept column (since rows 3-107 use this column for Supplier names).
...
for ($r = 3; $r -le $rowCount; $r++) {
    ...
    # 1. Try Distributor / Manufacturer first
    $provFolder = Find-ProviderFolder $distribuidor $fabricante
    
    # 2. Fallback: if distributor is empty/N/A, match the concept name in material column
    if (-not $provFolder) {
        $provFolder = Find-ProviderFolder $material $null
    }
...
Write-Output "Script generado con sintaxis y fallback corregido: $destPath"
