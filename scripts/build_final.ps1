$outputPath   = "C:\Users\User\Downloads\provedores\LIBRO MUESTRA.xlsx"

# Build the final corrected populate script
$script = @'
...
function Get-DriveRoot([string]$id, [string]$namePattern) {
    ...
    return if ($d) { $d.FullName } else { $null }
}
...
Write-Output "Script generado: $destPath"
